...
if ( $RUNNING_ON_WINDOWS && ($ENV{'SERVER_SOFTWARE'}=~ /IIS/) ) {
    $ENV{'PATH_INFO'} =~ s/^$ENV_SCRIPT_NAME// ;
}
...
if ( $is_html  && ($body ne '') && !$response_sent ) {
    ...
    #   Can it affect an initial <!DOCTYPE ...> tag, etc.?

    $body= "<!-- resource has been modified by proxy -->\n"
	  . &proxify_html(\$body, 1) ;
...
close(S) ;
untie(*S) ;
alarm(0) unless $RUNNING_ON_WINDOWS ;
...
sub proxify_html {
    ...
    splice(@out, ($body_pos || $html_pos), 0, &full_insertion($URL,0))
	if $is_full_page && $doing_insert_here ;
...
BEGIN {
	...
	my(undef, $len, $offset)= @_ ;   # $_[0] is handled explicitly below
	my($read, $errs) ;
...
sub newsocketto {
    ...
    # Note that gethostbyname() handles both alpha names and IP addresses.
...
<table width="100%"><tr>
<td align=left>
<a href="http://www.jmarshall.com/tools/cgiproxy/"><i>CGIProxy 2.0</i></a>
</td>
<td align=right>
$rightlink
</td>
