    ...
    public partial class Fonts : UserControl
    {
        ...
        private void OnClickExport(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                MessageBox.Show(String.Format("Character saved to {0}", filename), "Saved");
            }
        }
        ...
        private void OnClickSave(object sender, EventArgs e)
        {
            ...
            if ((int)treeView.SelectedNode.Parent.Tag == 1)
            {
                ...
                MessageBox.Show(String.Format("Unicode saved to {0}", FileName), "Save");
            }
            else
            {
                string FileName = Path.Combine(path, "fonts.mul");
                ASCIIText.Save(FileName);
                MessageBox.Show(String.Format("Fonts saved to {0}", FileName), "Save");
            }
        }
