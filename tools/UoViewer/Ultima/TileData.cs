...
namespace Ultima
{
	...
	public struct ItemData
	{
		...
		public ItemData( string name, TileFlag flags, int weight, int quality, int quantity, int value, int height, int anim, int hue, int stackingoffset )
		{
			m_Name = name;
			m_Flags = flags;
			...
			if ( filePath != null )
			{
				using ( FileStream fs = new FileStream( filePath, FileMode.Open, FileAccess.Read, FileShare.Read ) )
				{
					...
					for ( int i = 0; i < 0x4000; ++i )
					{
						...
						m_ItemData[i] = new ItemData( ReadNameString( bin ), flags, weight, quality, quantity, value, height, anim, hue, stackingoffset );
